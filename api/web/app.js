...
function renderDetail() {
  ...
  for (const p of (it.panels || []).filter((p) => p.bitmap)) {
    ...
    d.appendChild(wrap);
    {
      ...
      // download the panel image AS CURRENTLY PROCESSED (a rotate replaces
      // p.file, so the URL is minted at click time, never cached)
      const dl = document.createElement("a");
      dl.dataset.role = `download-${p.panel}`;
      dl.className = "btn btn-sm btn-outline gap-1 mb-2 ml-2";
      dl.href = "#";
      dl.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-[1.1em]" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>' +
        `Download ${p.panel}`;
      dl.setAttribute("aria-label", `Download the ${p.panel} label image (${p.file.name})`);
      dl.addEventListener("click", (e) => {
        e.preventDefault();
        const url = URL.createObjectURL(p.file);
        const tmp = document.createElement("a");
        tmp.href = url;
        tmp.download = p.file.name || `${p.panel}-label.jpg`;
